...
class SiteSettingController extends Controller
{
    ...
    public function editSiteSetting()
    {
        ...
        $mail_drivers = ['smtp' => 'SMTP', 'mail' => 'Mail', 'sendmail' => 'SendMail', 'mailgun' => 'MailGun', 'mandrill' => 'Mandrill', 'ses' => 'Amazon SES', 'sparkpost' => 'Sparkpost', 'log' => 'Log'];
        $siteSetting = SiteSetting::findOrFail($id);
        return view('admin.site_setting.edit')->with('siteSetting', $siteSetting)->with('mail_drivers', $mail_drivers)->with('countries', $countries)->with('currency_codes', $currency_codes);
    }
    public function updateSiteSetting(SiteSettingFormRequest $request)
    {
        ...
        if ($request->hasFile('image')) {
            ...
            $fileName = ImageUploadingHelper::UploadImage('sitesetting_images', $request->file('image'), $image_name);
    ...
    private function deleteSiteSettingImage($id)
    {
        try {
            ...
            if (!empty($image)) {
                File::delete(ImageUploadingHelper::real_public_path() . 'sitesetting_images/thumb/' . $image);
                File::delete(ImageUploadingHelper::real_public_path() . 'sitesetting_images/mid/' . $image);
                File::delete(ImageUploadingHelper::real_public_path() . 'sitesetting_images/' . $image);
            }
            return 'ok';
